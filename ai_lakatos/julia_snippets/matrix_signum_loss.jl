...
struct matrixSignumLoss <: Function
...
function matrixSignumLoss(requiredOps=[(1, 2), (2, 3), (2, 4)]; complexityWeight=0.5, unusedFunctionPenalty=1e1)
    return matrixSignumLoss(requiredOps, complexityWeight, unusedFunctionPenalty)
end

function (loss::matrixSignumLoss)(tree, dataset::Dataset{T,L}, options, idx) where {T,L}
    X = copy(dataset.X)
    y = copy(dataset.y)
